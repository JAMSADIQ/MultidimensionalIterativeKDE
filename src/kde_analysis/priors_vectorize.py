...
def chi_effective_prior_from_aligned_spins(q, aMax, xs):
    ...
    Inputs
    q: Mass ratio value or values (according to the convention q<1)
    ...
    # Ensure that q is an array and not >1
    q = np.reshape(q, -1)
    assert not(q.max() > 1), "Some of your q values are >1!"
    ...
    q_A, x_A = q[caseA], xs[caseA]
    q_B, x_B = q[caseB], xs[caseB]
    q_C, x_C = q[caseC], xs[caseC]
...
def chi_effective_prior_from_isotropic_spins(q, aMax, xs):
    ...
    # Set up various piecewise cases
    pdfs = np.ones(xs.size, dtype=complex) * (-1.)  # sentinel value for edge cases
    ...
    q_B, x_B = q[caseB], xs[caseB]
    q_C, x_C = q[caseC], xs[caseC]
    q_D, x_D = q[caseD], xs[caseD]
    q_E, x_E = q[caseE], xs[caseE]
    ...
    pdfs[caseA] = (
        ...
        * (
            ...
            * (
                4. + 2. * np.log(aMax)
                - np.log(q_A**2 * aMax**2 - (1. + q_A)**2 * x_A**2)
            )
            - 2. * (1. + q_A) * x_A * np.arctanh((1. + q_A) * x_A / (q_A * aMax))
            + (1. + q_A) * x_A
              * (Di(-q_A * aMax / ((1. + q_A) * x_A))
                - Di(q_A * aMax / ((1. + q_A) * x_A)))
        )
    )
    ...
    pdfs[caseC] = (
        ...
        * (
            ...
            + (aMax + (1. + q_C) * x_C * np.log((1. + q_C) * x_C))
              * np.log(q_C * aMax / (aMax - (1. + q_C) * x_C))
            - (1. + q_C) * x_C * np.log(aMax)
              * (2. + np.log(q_C) - np.log(aMax - (1. + q_C) * x_C))
            + q_C * aMax * np.log(aMax / (q_C * aMax - (1. + q_C) * x_C))
            + (1. + q_C) * x_C
              * np.log((aMax - (1. + q_C) * x_C) * (q_C * aMax - (1. + q_C) * x_C) / q_C)
            + (1. + q_C) * x_C
              * (Di(1. - aMax / ((1. + q_C) * x_C))
                - Di(q_C * aMax / ((1. + q_C) * x_C)))
        )
    )
        ...
        * (
            ...
            - x_D * np.log(aMax)
              * (
                  2. * (1. + q_D)
                  - np.log((1. + q_D) * x_D)
                  - q_D * np.log((1. + q_D) * x_D / aMax)
              )
            + (1. + q_D) * x_D
              * np.log((-q_D * aMax + (1. + q_D) * x_D) * (aMax - (1. + q_D) * x_D) / q_D)
            + (1. + q_D) * x_D
              * np.log(aMax / ((1. + q_D) * x_D))
              * np.log((aMax - (1. + q_D) * x_D) / q_D)
            + (1. + q_D) * x_D
              * (Di(1. - aMax / ((1. + q_D) * x_D))
                - Di(q_D * aMax / ((1. + q_D) * x_D)))
        )
    )
    ...
    pdfs[caseE] = (
        ...
        * (
            ...
            + (1. + q_E) * x_E
              * np.log(((1. + q_E) * x_E - aMax) * ((1. + q_E) * x_E - q_E * aMax) / q_E)
            + (1. + q_E) * x_E
              * np.log((1. + q_E) * x_E)
              * np.log(q_E * aMax / ((1. + q_E) * x_E - aMax))
            - q_E * aMax * np.log(((1. + q_E) * x_E - q_E * aMax) / aMax)
            + (1. + q_E) * x_E
              * (Di(1. - aMax / ((1. + q_E) * x_E))
                - Di(q_E * aMax / ((1. + q_E) * x_E)))
        )
    )
    ...
    pdfs[caseA] = (
        ...
        * (
            ...
            * (
                aMax * q * (3. + 4. * q) / (4. + 3. * q)
                - np.sqrt(
                    aMax**2 * q**2 * ((3. + 4. * q) / (4. + 3. * q))**2 - x_A**2
                )
...
def joint_prior_from_isotropic_spins(
    ...
    p_chi_p_given_chi_eff = np.array(
        [
            chi_p_prior_given_chi_eff_q(q, aMax, xeffs[i], xps[i], ndraws, bw_method)
            for i in range(len(xeffs))
        ]
    )
