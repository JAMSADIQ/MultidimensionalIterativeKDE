...
fm1 = h5.File('Final_noncosmo_GWTC3_m1srcdatafile.h5', 'r')
d1 = fm1['randdata']

#f2dL = h5.File(opts.datafilename2, 'r')
f2dL = h5.File('Final_noncosmo_GWTC3_dL_datafile.h5', 'r')
d2 = f2dL['randdata']
...
ymin, ymax = 200, 8000 #new limits
Npoints = opts.Npoints #200 bydefault
#we need log space points for m1, linear in dL
p1grid = np.logspace(np.log10(xmin), np.log10(xmax), 200)#Npoints)
p2grid = np.linspace(ymin, ymax, 150)

#XX, YY = np.meshgrid(p1grid, p2grid)
XX, YY = np.meshgrid(p1grid, p2grid, indexing='ij') #use of indexing to make sure we are correct
...
if opts.fpopchoice == 'rate':
    ...
    for i, m1val in enumerate(m1_source_grid):
        for j, dLval in enumerate(dL_grid):
            zcosmo = z_at_value(cosmo.luminosity_distance, dLval*u.Mpc).value
            m1det = m1val*(1.0 + zcosmo)
            pdet2D[i, j] = u_pdet.pdet_of_m1_dL_powerlawm2(m1det, 5.0, dLval, beta=1.26, classcall=g)

capped_pdet2D = np.maximum(pdet2D, opts.Maxpdet)
u_plot.plot_pdet2D(XX, YY, pdet2D.T, Maxpdet=opts.Maxpdet, pathplot=opts.pathplot, show_plot=False)
#get rates
...
frateh5 = h5.File(opts.output_filename+'UsedL2priorfactor_uniform_prior_mass_2Drate_m1'+opts.parameter2+'max_pdet_cap_'+str(opts.Maxpdet)+'.hdf5', 'w')
...
frateh5.create_dataset('pdet2DNotranspose', data=pdet2D)
frateh5.create_dataset('pdet2DwithcapNotranspose', data=capped_pdet2D)
...
for i in range(Total_Iterations + discard):
    ...
    if i >= discard + Nbuffer:
        ...
        buffer_interp = RegularGridInterpolator((p1grid, p2grid), buffer_kdes_mean, bounds_error=False, fill_value=0.0)
    rwsamples = []
...
for val in [300, 500, 600, 900, 1200, 1500, 1800, 2100, 2400, 2700, 3000, 3500, 4000]:
