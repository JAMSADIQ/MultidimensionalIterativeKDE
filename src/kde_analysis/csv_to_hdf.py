...
def process_and_save_to_hdf(csv_filepath, hdf_filepath):
    ...
    df = pd.read_csv(csv_filepath, header=None, names=['m1', 'm2', 'Xieff', 'VT'])
    ...
    if actual_size == expected_size:
        ...
        VT_3Dgrid = df['VT'].values.reshape((len(m1grid), len(m2grid), len(Xigrid)))
    else:
            ...
            for j in range(len(m2grid)):
                for k in range(len(Xigrid)):
                    if (i, j, k) in data_dict:
                        ...
                        raise ValueError(f"Missing data for m1={m1grid[i]}, m2={m2grid[j]}, Xieff={Xigrid[k]} and no symmetric point available")
    
    # Save to HDF5 file
    with h5py.File(hdf_filepath, 'w') as hf:
        hf.create_dataset('m1grid', data=m1grid)
        hf.create_dataset('m2grid', data=m1grid)  # As per your note, m2grid = m1grid
        hf.create_dataset('Xieffgrid', data=Xigrid)
