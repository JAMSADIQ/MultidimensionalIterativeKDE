...
for ik, dLval in  enumerate(dLarray):#[300, 500, 800, 1200, 1500, 1800, 2100, 2500, 3000, 3500, 4000, 4500, 5000]):
    ...
    ax.plot(p1grid, rate50, color=color,  lw=1.5, label=f'z={zarray[ik]:.1f}')
...
ax.set_xlim(7, 80)
#ax.set_ylim(ymin=1e-11)
ax.set_ylim(ymin=1e-4, ymax=2)
#ax.set_yticks([])  
plt.tight_layout()
plt.savefig('median_rate_m1_redshiftcolor_m1m2dLanalysis_pdetcap_01_Xieffmaxbw_dL_03.png'.format(dLval), bbox_inches='tight')

plt.semilogx()
plt.tight_layout()
plt.savefig('median_rate_m1_redshiftcolor_m1m2dLanalysis_pdetcap_01_Xieffmaxbw_dL_03LogX_axis.png', bbox_inches='tight')
plt.show()
