...
dLMpc = 5000 #Mpc
#get redshift
z = z_at_value(cosmo.luminosity_distance, dLMpc*u.Mpc).value
DcAstro = cosmo.comoving_distance(z).value  #astropy way
Dccode = comoving_distance(z, H0, omega_m)
print(DcAstro, Dccode)
Omega_Lambda = 1.0 - omega_m
Ez = np.sqrt(omega_m * (1 + z)**3 + Omega_Lambda)
dH = c/H0
ddldz = dLMpc/(1+z) +(1+z)*dH/Ez
...
print(ddldz, get_ddL_bydz_factor(dLMpc)[1])

quit()
dDcdz = comoving_distance_derivative(z, H0, omega_m)
dVcdz =  4.0 * np.pi * (cosmo.differential_comoving_volume(z)).to(u.Gpc**3/u.sr).value

def get_dV_c_dz(dL_Mpc):
    """
    for flat cosmology
    Vc = 4pi/3 Dc^3 (Dc = comoving disatnce)
    dVc/dz = 4pi Dc  dDc/dz
    we can get Dc and dDc/dz defined above
    """
    z_at_dL = z_at_value(cosmo.luminosity_distance, dL_Mpc*u.Mpc).value
    D_c = comoving_distance(z_at_dL, H0, omega_m)
    dD_c_dz = comoving_distance_derivative(z_at_dL, H0, omega_m)
    dVcdz = 4 * np.pi * D_c**2 *dD_c_dz 
    #convert to Gpc3
    dVcdzGp3 = dVcdz/1000**3
    #return dVcdzGp3 
    return dVcdz

dVcode = get_dV_c_dz(dLMpc)
